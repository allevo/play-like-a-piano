 ...
 * assets), so we just drop the markup into the DOM and let it loop. Colours are
 * hard-coded to the app palette because CSS custom properties do not reach
 * inside markup injected via innerHTML.
 */
...
function svg(inner: string): string {
  return (
    ...
    `xmlns="http://www.w3.org/2000/svg" class="phys-svg" ` +
    `font-family="system-ui, -apple-system, sans-serif">${inner}</svg>`
  );
}

function label(x: number, y: number, text: string, color = MUTED, size = 12): string {
  return `<text x="${x}" y="${y}" fill="${color}" font-size="${size}">${text}</text>`;
}

/** Path for one string mode: `n` half-waves of amplitude `amp`, around y = 0. */
function modePath(width: number, amp: number, n: number, samples = 60): string {
  const pts: string[] = [];
 ...
 */
function yScale(values: string): string {
  return values
    .split(";")
    .map((v) => `1 ${v.trim()}`)
    .join(";");
